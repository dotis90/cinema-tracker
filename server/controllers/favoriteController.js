...
exports.checkIfFavorite = (req, res) => {
  Favorite.find({ movieId: req.body.movieId, userFrom: req.user.id }).exec((err, favorite) => {
    if (err) return res.status(400).send(err);
...
exports.removeFromFavorites = (req, res) => {
  Favorite.findOneAndDelete({ movieId: req.body.movieId, userFrom: req.user.id }).exec((err, doc) => {
...
exports.getAllFavorites = (req, res) => {
  Favorite.find({ userFrom: req.user.id }).exec((err, favorites) => {
    if (err) return res.status(400).send(err);
